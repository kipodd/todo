...
while True:
    ...
    if event == "add" and (new_todo := sg.popup_get_text("Add task:", no_titlebar=True)) \
            and new_todo not in set(todos):
        todos.append(new_todo)
        window["todos"].Update(values=todos)
    elif event == "edit" and (selected := values["todos"] and values["todos"][0]) \
            and (new_todo := sg.popup_get_text("Edit task:", no_titlebar=True)) \
            and new_todo not in set(todos):
        todos = [t if t != selected else new_todo for t in todos]
        window["todos"].Update(values=todos)
